...
Module.register("MMM-DarkSkyForecast", {
  ...
  */
  processWeatherData: function() {
    ...
    return {
      "currently" : {
        temperature: Math.round(this.weatherData.currently.temperature) + "°F",
  ...
  forecastItemFactory: function(fData, type) {
    ...
    if (type == "hourly") { //just display projected temperature for that hour
      fItem.temperature = Math.round(fData.temperature) + "°F";
  ...
  formatHiLowTemperature: function(h,l) {
    return {
      high: (!this.config.concise ? this.config.label_high + " " : "") + Math.round(h) + "°F",
      low: (!this.config.concise ? this.config.label_low + " " : "") + Math.round(l) + "°F"
    };
  },
   ...
   */
  iconsets: {
    "1m": {path:"1m", format:"svg"}, /* copy and recolor of weather icons to match WDWToday */
